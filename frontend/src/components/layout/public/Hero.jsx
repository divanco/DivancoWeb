...
const Hero = ({
  ...
  const mediaUrl = backgroundImage || data?.data?.url || DEFAULT_HERO;
  const mediaType = backgroundImage
    ? 'image'
    : (data?.data?.type === 'video' && data?.data?.url ? 'video' : 'image');
  ...
  }, [isPanning, isMobile, panPosition, isVideo]);
